        ...
        for product in rows_lst:
            if product[0] == counter + 1:
                ...
                if prod_art:
                    product_description = f'{product_description}арт. {prod_art}'
                if prod_barcode:
                    prod_uin, string_number = find_uin(prod_barcode, giis_list)
                    if prod_uin:
                        product_description = f'{product_description}, уин {prod_uin}'
                product_description = f'{product_description}, вес {prod_weight} г.'
...
# invoice_parsing(
#     'E:\Elena\Documents\Документы  Александрова Е.П\Накладные\Входящие накладные\Белышева Юлия\Аена 99.xlsx')
